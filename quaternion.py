...
def quaternion_product(ql: np.ndarray, qr: np.ndarray) -> np.ndarray:
    ...
    M = np.zeros((4,4))
    M[0,1:] = -epsilon_left.T
    M[1:,0] = epsilon_left.T
    M[1:,1:] = utils.cross_product_matrix(epsilon_left)
...
def quaternion_to_euler(quaternion: np.ndarray) -> np.ndarray:
    ...
    e_1_squared = quaternion_squared[1]
    e_2_squared = quaternion_squared[2]
    e_3_squared = quaternion_squared[3]
    
    #raise NotImplementedError  # TODO: remove when done
    phi = np.arctan2(2*(e_3*e_2+eta*e_1), eta_squared - e_1_squared - e_2_squared + e_3_squared)  # TODO: Convert from quaternion to euler angles
    theta = np.arcsin(2*(eta*e_2-e_1*e_3))  # TODO: Convert from quaternion to euler angles
    psi = np.arctan2(2*(e_1*e_2+eta*e_3), eta_squared + e_1_squared - e_2_squared - e_3_squared)  # TODO: Convert from quaternion to euler angles
