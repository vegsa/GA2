...
class ESKF:
    ...
    def predict_nominal(
        ...
        accel = R @ acceleration + self.g
        position_prediction = position + Ts*velocity + (Ts**2/2)*accel  # TODO: Calculate predicted position
        velocity_prediction = velocity + Ts*accel  # TODO: Calculate predicted velocity

        kappa = Ts*omega
        kappa_norm = la.norm(kappa)
        e_kappa_T = np.array([np.cos(kappa_norm/2), *(np.sin(kappa_norm/2))*kappa.T/kappa_norm])
        quaternion_prediction = quaternion_product(quaternion, e_kappa_T)  # TODO: Calculate predicted quaternion
    ...
    def Gerr(self, x_nominal: np.ndarray,) -> np.ndarray:
        ...
        G = np.zeros((15, 12))
        block_G = la.block_diag(-R,-I, I, I)
        G = np.vstack((np.zeros((3,12)), block_G))
    ...
    def discrete_error_matrices(
        ...
        V[CatSlice(start=0, stop=15)*CatSlice(start=15, stop=30)] = (G @ self.Q_err @ G.T)
        V[CatSlice(start=15, stop=30)*CatSlice(start=15, stop=30)] = A.T
    ...
    def inject(
        ...
        # Done in the same way as in eqn 10.72.        
        x_injected[INJ_IDX] = x_nominal[INJ_IDX] + delta_x[DTX_IDX]
        q = quaternion_product(x_nominal[ATT_IDX], np.concatenate(([1], (0.5*delta_x[ERR_ATT_IDX]))))
        q = q/la.norm(q)
        x_injected[ATT_IDX] = q
        
        # Covariance
        G_injected = np.eye(15)  # TODO: Compensate for injection in the covariances
    ...
    def innovation_GNSS_position(
        ...
        H_x = (np.concatenate((I, np.zeros((3,13))), axis=1))
        ...
        v = z_GNSS_position - H_x @ x_nominal  # TODO: innovation
    ...
    def update_GNSS_position(
        ...
        H_x = np.concatenate((np.identity(3), np.zeros((3,13))), axis=1)
    ...
    def delta_x(cls, x_nominal: np.ndarray, x_true: np.ndarray,) -> np.ndarray:
        ...
        quaternion_conj = np.diag([1, -1, -1, -1]) @ q # q_conj = a - bi - cj - dk 
        
        delta_quaternion = quaternion_product(quaternion_conj, x_true[ATT_IDX])/np.linalg.norm(quaternion_product(quaternion_conj, x_true[ATT_IDX]))  # TODO: Error quaternion
    ...
    def NEESes(
        ...
        NEES_att = d_x[ERR_ATT_IDX].T @ la.inv(P[ERR_ATT_IDX**2]) @ d_x[ERR_ATT_IDX]  # TODO: NEES attitude
        NEES_accbias = d_x[ERR_ACC_BIAS_IDX].T @ la.inv(P[ERR_ACC_BIAS_IDX**2]) @ d_x[ERR_ACC_BIAS_IDX]  # TODO: NEES accelerometer bias
    ...
    def _NEES(cls, diff, P):
        NEES = diff @ la.solve(P, diff)  # TODO: NEES, kan brukes i funksjonen over.
